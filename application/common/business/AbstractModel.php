...
abstract class AbstractModel {
     ...
     * @param array $param
     */
    protected static function _checkPageAndCount(&$params) {
        $params['page'] = !isset($params['page']) ? 1 : $params['page'];
        $params['count'] = !isset($params['count']) ? 8 : $params['count'];
...
//}
     ...
     * 转成字母
     */
    public static function getfirstchars($s0) {
        $fchar = ord($s0{0});
     ...
     * @return string             拼音字符串
     */
    public static function getfirstchar($str, $ret_format = 'one', $placeholder = '_', $allow_chars = '/[a-zA-Z\d ]/') {
        static $pinyins = null;
    ...
    public static function encrypt_phone($phone) {
        if (!$phone) {
            exception('手机号码不能为空', 110015);
        }
        //1.换位号码中两组数字：第二位和第六位交换，第三位和第五位交换 将全部号码转换为对应的字符
        $phone = self::encry_phone($phone);
     ...
     * @return string(11) 加密后的手机号
     */
    public static function decrypt_phone($phone) {
        if (!$phone) {
            exception('手机号码不能为空', 110015);
        }
        //3.将剩余字母全部转换为对应的数字
        $phone = self::decry_phone($phone);
     ...
     * @param  $end_num  4位尾号
     * @return string（11) 加密后的手机号码
     */
    public static function encry_phone($phone) {
        $phonearray = str_split($phone);
        //将全部号码转换为对应的字符
        $getPhoneStr = self::getPhoneStr();
        $returnphnoe = self::change($phonearray, $getPhoneStr);
        return $returnphnoe;
    }

    /**
     * getPhoneNum($encodeEnd)
     * @param  $encodeEnd 加密后手机号
     */
    public static function decry_phone($phone) {
        $phonearray = str_split($phone);
        //将对应的字符转换为数值
        $getPhoneStr = array_flip(self::getPhoneStr());
        $returnphnoe = self::change($phonearray, $getPhoneStr);
        return $returnphnoe;
    }

    /**
     *  getPhoneStr($mid_num, $snum, $id)
     *  数字转字母
     * @return array() 数组
     */
    public static function getPhoneStr() {
        return array(
            0 => 'b',
            1 => 'a',
            2 => 'c',
    ...
    /**
     * getPhoneNum($encodeEnd)
     * @param  $phonearray  号码数组
     * @param  $getPhoneStr 编码数组
     * @param  $returnphnoe 加密后手机号
     */
    public static function change($phonearray, $getPhoneStr) {
        $returnphnoe = '';
        foreach ($phonearray as $key => $val) {
            if (!isset($getPhoneStr[$val])) {
                exception('传入参数信息错误', 100004);
            }
            if ($key == 1 || $key == 2 || $key == 4 || $key == 5) {
                if ($key == 1)
                    $returnphnoe.=$getPhoneStr[$phonearray[5]];
                if ($key == 2)
                    $returnphnoe.=$getPhoneStr[$phonearray[4]];
                if ($key == 4)
                    $returnphnoe.=$getPhoneStr[$phonearray[2]];
                if ($key == 5)
                    $returnphnoe.=$getPhoneStr[$phonearray[1]];
            }else {
                $returnphnoe.=$getPhoneStr[$phonearray[$key]];
            }
        }
        return $returnphnoe;
    }
     ...
     * 七牛防盗链
     */
    public static function qiniuencrypt($url) {

        $data = parse_url($url);
        $key = Config::get('qiniu.KEY');
        $encrypttime = strtotime(date("Y-m-d h:i:s"));
        $str = md5($key . $data['path'] . $encrypttime);
        $url = $url . "?" . http_build_query(array('sign' => $str, 't' => $encrypttime));
        return $url;
    }
    ...
    public static function getAppVersion() {
        if (!isset($_SERVER['HTTP_APPVERSION']) || empty($_SERVER['HTTP_APPVERSION'])) {
            $app_version = '';
        } else {
            $app_version = $_SERVER['HTTP_APPVERSION'];
        }
        if ($app_version) {
     ...
     *
     */
    public static function getwebAppVersion() {
        $input=input();
        if (!isset($input['app_version']) || empty($input['app_version'])) {
            $app_version = '';
        } else {
    ...
    public static function curlRequest($url, $method = "GET", $params = array(), $timeout = 30, $extParams = array()) {
        ...
        curl_setopt($ch, CURLOPT_COOKIE, "cna=9tOgE3hzG18CAXgk+hSaootG; otherx=e%3D1%26p%3D*%26s%3D0%26c%3D0%26f%3D0%26g%3D0%26t%3D0; hng=CN%7Czh-CN%7CCNY%7C156; lid=tb3051505_11; t=d8be972269fe76fd471a88220fa3692c; uc3=vt3=F8dByRjNU5Dja2SnXik%3D&id2=WvA200nbI3Ja&nk2=F5RGNs%2Boybct5YgI&lg2=W5iHLLyFOGW7aA%3D%3D; tracknick=tb3051505_11; lgc=tb3051505_11; _tb_token_=e03635a45ee7f; cookie2=194d06882f957a47bdca080f34c711d5; tk_trace=1; _m_h5_tk=7e46ac8469d94ea3ef382082d7e491f5_1541078101245; _m_h5_tk_enc=b345aadb5c1a0108e1a6ad9b1a5dba4c; isg=BDc33zdfINQMLKS29kpI0Fy5xitBVAuZr-Z0c4nkU4ZtOFd6kcybrvUaHt7D0OPW");
